...
//  styled components
const CategoryBoxWrapper = styled(FlexRowCenter)<{ selected: any }>(({ selected, theme }) => ({
  flex: "1 1 0",
  height: "175px",
  margin: "0.75rem",
  minWidth: "200px",
...
const SalePage1 = () => {
  ...
  useEffect(() => {
    setProductList(productDB.slice(page * productPerPage, (page + 1) * productPerPage));
  }, [page]);
  ...
  const scroller = useCallback(() => {
    ...
    if (categoryRef.current) {
      postionHeight = categoryRef.current.offsetTop + categoryRef.current.offsetHeight;
    }
    ...
    <SaleLayout>
      <Container sx={{ mt: "2rem" }}>
            ...
            {saleCategoryList.map((item, ind) => {
              ...
              return (
                ...
                >
                  ...
                  <H5 color={selectedItem ? "primary.main" : "inherit"}>{item.title}</H5>
                  <StyledChip
                    size="small"
                    color="primary"
                    label="Upto 40% off"
        ...
        <FlexBetween flexWrap="wrap" my={8}>
          <Span>{renderProductCount(page, productPerPage, productDB.length)}</Span>
          <Pagination
            page={page}
            color="primary"
            variant="outlined"
