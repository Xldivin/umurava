...
const ParentNavItem = styled(Box)(({ theme }) => ({
  ...
  display: "none",
  position: "absolute",
  [theme.breakpoints.down(1640)]: { right: "100%", left: "auto" },
}));

const NavBarWrapper = styled(BazarCard)<{ line: number }>(({ theme, line }) => ({
  height: "60px",
  display: "block",
  borderRadius: "0px",
  position: "relative",
  ...(line === 0 && { borderBottom: `1px solid ${theme.palette.grey[200]}` }),
...
const Navbar: FC<NavbarProps> = ({ navListOpen, hideCategories, elevation }) => {
    ...
    return list.map((nav: NavList) => {
      if (isRoot) {
        ...
        if (nav.megaMenuWithSub) {
          //@ts-ignore
          return <MegaMenu2 key={nav.title} title={nav.title} menuList={nav.child} />;
        }
          ...
          return (
            ...
            >
              ...
              <ChildNavsWrapper className="child-nav-item">
                <BazarCard elevation={3} sx={{ mt: 2.5, py: 1, minWidth: 200 }}>
...
Navbar.defaultProps = { elevation: 2, navListOpen: false, hideCategories: false };
